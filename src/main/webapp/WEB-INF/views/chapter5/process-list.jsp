...
<table width="100%" class="table table-bordered table-hover table-condensed">
    ...
    <tr>
        ...
        <th>流程图</th>
<%--        <td>
            <a href="${ctx}/diagram-viewer/index.html?processDefinitionId=${task.processDefinitionId}&processInstanceId=${task.processInstanceId}">进度</a>
        </td>--%>
        <th width="40">状态</th>
        <th>候选启动</th>
        <th width="70">操作</th>
    ...
    <c:forEach items="${page.result }" var="pd" varStatus="row">
        <tr index="${row.index}">
            ...
            <td><a target="_blank" title="资源名称：${pd.resourceName }"
                   href='${ctx }/chapter5/read-resource?pdid=${pd.id }&resourceName=${pd.resourceName }'>查看</a></td>
<%--            <td><a target="_blank" title="资源名称：${pd.diagramResourceName }"
                   href='${ctx }/chapter5/read-resource?pdid=${pd.id }&resourceName=${pd.diagramResourceName }'>查看</a>
            </td>--%>
            ...
            <td>
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu" style="min-width: 100px;margin-left: -50px;">
                        ...
                        <li><a class="set-startable" data-pid="${pd.id}" title="设置候选启动人|组"><i class="icon-user"></i>候选启动</a></li>
                        ...
                        <c:if test="${pd.suspended}">
                            <li><a href="#changeStateModal" class="change-state" data-state="active" data-pid="${pd.id}"
                                   data-toggle="modal"><i class="icon-ok"></i>激活</a></li>
                        </c:if>
